  ...
  return (
    <>
      <header className="h-15 border-b border-slate-200 bg-white px-4 md:px-6 flex items-center justify-between shrink-0 dark:border-slate-800 dark:bg-slate-900 z-30">
        {/* Left: Mobile Menu Toggle & Agency ID */}
          ...
          {organization && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Click to copy Agency ID"
              >
                <Key className="h-3 w-3 text-indigo-500" />
                <span>Agency ID: {organization.joinCode}</span>
        ...
        {/* Right: Profile, Settings & Switcher */}
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          {/* Profile & Security Settings Button */}
          <button
            onClick={() => setIsProfileModalOpen(true)}
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 text-xs font-semibold text-slate-700 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-200 cursor-pointer transition-colors"
            title="Update Profile Name & Password"
          >
